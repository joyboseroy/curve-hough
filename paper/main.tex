...
\author{Joy Bose\thanks{Code, benchmark generator, and trained configurations: \url{https://github.com/joyboseroy/curve-hough}. Archived version: \url{https://doi.org/10.5281/zenodo.21424983}}\\Independent Researcher, Bengaluru, India\\\texttt{joy.bose@ieee.org}}
\date{}
...
\begin{itemize}
  ...
  \item \textbf{Benchmark.} A controlled, deterministically seeded synthetic benchmark generator covering parabolas, circles, and ellipses ($d=5$), with additive noise, occlusion, and distractor structures, and matched-compute baseline implementations: classical Hough, RANSAC, direct regression, and a DETR-style hypothesis-query head (Section~\ref{sec:experiments}).
...
The factorized cascade uses 26.9$\times$ less memory than the dense accumulator at $d=3$; by Proposition~1 this ratio grows with $d$, since the dense term scales as $B^{d}$ while the factorized term stays $O(B^{d_1} + kB^{d_2})$ for any fixed anchor dimensionality $d_1$. Table~\ref{tab:memory_scaling} measures this directly at our actual ellipse configuration ($d=5$): the dense accumulator's arithmetic size (not instantiated -- doing so is precisely the impracticality in question) grows $1{,}024\times$ from $d=3$ to $d=5$ at fixed $B=32$, from 4.00\,MB to 4.00\,GB, exceeding typical single-GPU memory outright; the factorized cascade's measured size grows only $1.6\times$, from 152\,KB to 236\,KB, because $d_1=2$ (the anchor) is fixed regardless of $d$ and only $d_2$ (the shape) grows with the family.

\begin{table}[htbp]
\centering
\caption{Accumulator memory scaling from $d=3$ (parabola) to $d=5$ (ellipse) at fixed $B=32$, $C=32$. Dense sizes for $d=5$ are computed arithmetically, not instantiated.}
\label{tab:memory_scaling}
\small
\begin{tabular}{lccc}
\toprule
& \textbf{Dense $O(B^{d})$} & \textbf{Factorized (measured)} & \textbf{Reduction} \\
\midrule
$d=3$ (parabola) & 4.00 MB & 152 KB & 26.9$\times$ \\
$d=5$ (ellipse)  & 4.00 GB & 236 KB & 17{,}772.5$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\emph{Remark (permutation invariance).} The voting operator $\mathbf{Y}(\mathbf{p}) = |\Omega(\mathbf{p})|^{-1} \sum_{i \in \Omega(\mathbf{p})} \mathbf{X}(i)$ is invariant to any permutation of the pixel indices within $\Omega(\mathbf{p})$, immediately from summation and the cardinality-normalizing constant both being order-independent over the finite index set $\Omega(\mathbf{p})$: Equation~\ref{eq:vote} depends only on the multiset $\{\mathbf{X}(i) : i \in \Omega(\mathbf{p})\}$, not traversal order. This property is what allows the gather-and-mean implementation (a fixed index bank per candidate curve) to be computed with no ordering assumptions and no positional encoding, unlike a learned-attention alternative over the same support.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/accumulator_schematic.pdf}
\caption{Dense accumulator (left) vs.\ the factorized cascade (right), drawn to scale with the measured element counts of Table~\ref{tab:memory_complexity}. The dense accumulator materializes every $(\text{anchor}, \text{shape})$ combination at once; the cascade computes a coarse 2D anchor map, retains a handful of peaks, and instantiates a dense shape accumulator only at each retained peak.}
...
\paragraph{Benchmark.} Images of size $128 \times 128$ (main-text results below use $64 \times 64$; see Limitations) containing one to three curves from a family (parabolas, circles, ellipses), rendered with stroke width 2, plus: Gaussian pixel noise, random occluding rectangles removing up to 40\% of curve support, and distractor line segments. Splits: 8,000 train, 1,000 val, 1,000 test per family. Figure~\ref{fig:benchmark_examples} shows representative clean and corrupted examples for each family.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/benchmark_examples.png}
\caption{Representative benchmark images, clean (top) and corrupted (bottom: noise, occlusion, distractor segments), for each curve family.}
...
Table~\ref{tab:parabola_interim} reports measured results for our method on the parabola family at $64 \times 64$ resolution, at both the standard corrupted setting (noise, occlusion, up to 3 distractor segments, 1--3 curves per image) and a clean control (no corruption, exactly one curve per image), both evaluated over 200 test images. Table~\ref{tab:circle_interim} reports the corrupted setting for circles at the same resolution (threshold chosen by the same sweep protocol). Table~\ref{tab:baseline_interim} reports the four baselines on the corrupted parabola setting. Table~\ref{tab:ellipse_interim} reports the higher-dimensional ellipse family ($d=5$) at both settings. % TODO: ellipse baselines, 128x128 scale-up, ablations i-v.
...
\begin{table}[htbp]
\centering
\caption{Factorized DHT on ellipses ($d=5$: $x_0, y_0, r_x, r_y, \phi$), $64\times64$. Thresholds chosen by sweep, best F reported per setting.}
\label{tab:ellipse_interim}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Setting} & \textbf{P} & \textbf{R} & \textbf{F} & \textbf{drop from clean} \\
\midrule
Clean control     & 0.690 & 0.672 & 0.681 & --- \\
Corrupted (main)   & 0.350 & 0.311 & 0.329 & $-0.352$ (51.7\% relative) \\
\bottomrule
\end{tabular}
\end{table}

Notably, the ellipse clean-control F (0.681) exceeds the parabola clean-control F (0.650, Table~\ref{tab:parabola_interim}) despite ellipse's substantially larger parameter space ($d=5$ vs.\ $d=3$) and 3-dimensional shape accumulator (vs.\ 1-dimensional for parabola and circle), supporting the claim that the factorization's clean-setting accuracy does not degrade with dimensionality on its own. The corrupted setting tells a different, and equally important, story: ellipse's absolute drop from clean to corrupted (0.352) is nearly three times parabola's (0.125, 19.2\% relative), so while factorized voting remains \emph{tractable} at $d=5$ (Proposition~1, and the fact that training and inference complete at all where a dense accumulator would not), its \emph{robustness to partial evidence} degrades faster as shape dimensionality grows. This is intuitive: an occluded arc of an ellipse constrains a joint $(r_x, r_y, \phi)$ far more weakly than an occluded arc of a circle constrains a single radius, so the same occlusion fraction removes proportionally more identifying evidence at higher $d$. We report this as an honest limitation rather than a result to be smoothed over; the coarse-to-fine re-voting refinement described but not yet implemented (Section~\ref{sec:method}) is a plausible mitigation, since it would let the model recover shape precision from whatever partial evidence survives occlusion rather than relying on a single coarse-to-dense pass. % TODO: ellipse baselines (RANSAC 5-point conic fit is the key comparison) to establish whether this degradation is specific to our method or a property of the corrupted setting at this dimensionality generally.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.32\textwidth]{figures/stage1_heatmap_ellipse.png}
\includegraphics[width=0.32\textwidth]{figures/stage2_profile_ellipse.png}
\includegraphics[width=0.32\textwidth]{figures/qualitative_ellipse.png}
\caption{The same three views as Figure~\ref{fig:qualitative_parabola}, for a corrupted-setting ellipse example with two overlapping curves and partial occlusion. In contrast to the parabola case, the Stage-1 anchor accumulator (left) is diffuse rather than sharply peaked, since an ellipse's center is far more weakly indicated by summed features than a parabola's vertex; the Stage-2 shape profile (center) is noisy and multimodal, with the predicted peak landing well away from the true shape bin; and the resulting detections (right) visibly undersize and mislocalize both ellipses. This example illustrates, rather than merely states, the robustness gap discussed above (Table~\ref{tab:ellipse_interim}).}
\label{fig:qualitative_ellipse}
\end{figure}

\paragraph{Qualitative results.}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.32\textwidth]{figures/stage1_heatmap_parabola.png}
\includegraphics[width=0.32\textwidth]{figures/stage2_profile_parabola.png}
\includegraphics[width=0.32\textwidth]{figures/qualitative_parabola.png}
\caption{Stage-1 anchor accumulator (left), Stage-2 shape profile at the top peak (center), and the resulting detection against ground truth (right), for a representative parabola test image. The anchor peak lands on the true vertex and the shape profile is sharply peaked near the true curvature bin, giving a near-pixel-tight fit.}
\label{fig:qualitative_parabola}
\end{figure}
...
\section{Limitations}
Our benchmark is synthetic; transfer to natural and medical images is untested. All main-text results are at $64 \times 64$ resolution rather than the $128 \times 128$ the benchmark supports, a smaller-scale stand-in adopted for compute budget during rapid iteration; qualitative trends (parabola vs.\ ellipse degradation, geometric vs.\ free-form baselines) are expected to hold at $128 \times 128$ but have not yet been confirmed there. The factorization requires a spatially meaningful anchor, which exists for conics and vertex-form polynomials but not obviously for every family. Stage 2 conditions on stage 1 peaks, so anchor misses are currently unrecoverable; the coarse-to-fine re-voting refinement described but not yet implemented in Section~\ref{sec:method} is the most direct planned mitigation.
...
The present work should be viewed as establishing the feasibility of differentiable Hough voting beyond straight lines, evidenced concretely by the $1{,}024\times$ vs.\ $1.6\times$ memory scaling gap between dense and factorized accumulators from $d=3$ to $d=5$ (Table~\ref{tab:memory_scaling}), rather than as claiming state-of-the-art curve detection. The central contribution is the factorized voting formulation itself, which lets an end-to-end feature-space Hough transform's computational cost scale with conditional parameter subspaces instead of the full parameter dimensionality. We hope this provides a foundation for future learned voting architectures and higher-dimensional geometric hypothesis spaces.
...
\bibitem[Torrente and Beltrametti(2014)]{torrente2014} M.-L. Torrente and M.~C. Beltrametti. Almost-vanishing polynomials and an application to the Hough transform. \emph{Journal of Algebra and its Applications}, 13(8):1450057, 2014.
\bibitem[Massone et~al.(2015)]{massone2015} A.~M. Massone, A.~Perasso, C.~Campi, and M.~C. Beltrametti. Profile detection in medical and astronomical images by means of the Hough transform of special classes of curves. \emph{Journal of Mathematical Imaging and Vision}, 51(2):296--310, 2015.
...
\bibitem[Zhang et~al.(2023)]{houglanenet2023} J.-Q. Zhang, H.-B. Duan, J.-L. Chen, A.~Shamir, and M.~Wang. HoughLaneNet: lane detection with deep Hough transform and dynamic convolution. \emph{Computers \& Graphics}, 2023. arXiv:2307.03494.
...
\bibitem[Chen et~al.(2023)]{bsnet2023} H.~Chen, M.~Wang, and Y.~Liu. BSNet: lane detection via draw B-spline curves nearby. \emph{arXiv:2301.06910}, 2023.
